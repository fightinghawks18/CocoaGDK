...
cco_result get_result_from_errno(const i32 err) {
    switch (err) {
    case EADDRINUSE: {
...
long send_to_socket(cco_socket socket, const void *packet_data, const ssize packet_size, const char *ip,
                   const u16 port) {
    struct sockaddr_in addr = construct_address(socket->af, ip, port);
...
#define IS_SOCKET(socket) (socket->fd >= 0)

#define LOG_UNIX_ERR(msg)                                                                                              \
    do {                                                                                                               \
        const i32 err_code = errno;                                                                                    \
        cco_result err = get_result_from_errno(err_code);                                                              \
...
cco_result cco_socket_init(void) {
    // Unix doesn't need initialization like Winsock
    return CCO_SUCCESS;
}

void cco_socket_quit(void) { 
    // Unix doesn't need cleanup like Winsock
}
...
cco_result cco_socket_open(cco_socket_protocol socket_protocol, cco_socket *out_socket) {
    ...
    if (!IS_SOCKET(sock)) {
        free(sock);
        LOG_UNIX_ERR("Failed to create socket!");
    }
...
cco_result cco_socket_bind_addr(cco_socket socket, const char *ip, const u16 port) {
    if (SOCKET_BIND(socket, ip, port) != 0) {
        LOG_UNIX_ERR("Failed to bind address!");
    }
    return CCO_SUCCESS;
}
...
cco_result cco_socket_listen(cco_socket socket, i32 max_connection_requests) {
    if (SOCKET_LISTEN(socket, max_connection_requests) != 0) {
        LOG_UNIX_ERR("Failed to listen on socket!");
    }
    return CCO_SUCCESS;
}
...
cco_result cco_socket_accept(cco_socket socket, cco_socket *out_client) {
    ...
    if (!IS_SOCKET(client)) {
        free(client);
        LOG_UNIX_ERR("Failed to accept socket!");
    }
...
cco_result cco_socket_connect(cco_socket socket, const char *ip, u16 port) {
    if (SOCKET_CONNECT(socket, ip, port) != 0) {
        LOG_UNIX_ERR("Failed to connect to address!");
    }
    return CCO_SUCCESS;
}
...
cco_result cco_socket_send(cco_socket socket, const void *packet, const ssize packet_size) {
    if (SOCKET_SEND(socket, packet, packet_size) < 0) {
        LOG_UNIX_ERR("Failed to send packet!");
    }
    return CCO_SUCCESS;
}
...
cco_result cco_socket_leave_multicast_group(cco_socket socket) { 
    return SOCKET_LEAVE_MULTICAST(socket); 
}
...
cco_result cco_socket_recv(cco_socket socket, void *out_packet, const ssize max_packet_size, ssize *out_bytes_recv) {
    const long packet_size = SOCKET_RECV(socket, out_packet, max_packet_size);
    if (packet_size > 0) { // Successfully accessed packet
        *out_bytes_recv = (ssize)packet_size;
        return CCO_SUCCESS;
        ...
        *out_bytes_recv = 0;
        LOG_UNIX_ERR("Error receiving packets!");
    }
}
...
cco_result cco_socket_recv_from(cco_socket socket, char *out_ip, u16 *out_port, void *out_packet, ssize max_packet_size,
    ...
    const long packet_size = SOCKET_RECV_FROM(socket, addr, addr_len, out_packet, max_packet_size);
    if (packet_size > 0) { // Successfully accessed packet
        ...
        *out_port = 0;
        *out_bytes_recv = 0;
        LOG_UNIX_ERR("Error receiving packets with sender information!");
    }
}
...
void cco_socket_write_str(unsigned char *packet_data, usize *offset, const char *str) {
    ...
    cco_socket_write_u32(packet_data, offset, (u32)str_length);
